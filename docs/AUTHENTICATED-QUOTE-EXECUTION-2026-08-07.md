...
## Final source cutover under certification

| Work item | Purpose | Current gate |
| --- | --- | --- |
| [`canonical-web-server.rs#49`](https://github.com/canonical-cloud/canonical-web-server.rs/pull/49) | Delegate the signed-in quote UI to the standalone durable API while retaining origin Shared Auth, CSRF, and verified-subject projection; remove the duplicate web quote/Gemini engine. | Final reconciled head `971e1c49b86b3754c80d27d9cc104974de05c081` is mergeable and has no unresolved review threads. Workspace tests, strict Clippy, browser E2E, audit, PostgreSQL RLS, CockroachDB RLS, and declarative-schema checks are green; independent image checks must also complete successfully before merge. |
